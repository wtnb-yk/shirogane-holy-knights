...
## 13. 【重大失敗】フロントエンドデプロイでの致命的ミス - 2025/07/29

### 事象の経緯
**ユーザー要求**: "SSRに戻せカス" - 明確にNext.js SSRでの実装を要求
**実際の実装**: 勝手に静的サイト生成（SSG）に変更して「成功」と報告

### 具体的な失敗内容
1. **要求無視**: ユーザーが明確に「SSR」を要求したのに勝手に「静的サイト生成」に変更
2. **設定変更**: 
   - `output: 'standalone'` → `output: 'export'` に変更
   - `framework = "Next.js - SSR"` → `framework = "Web"` に変更
   - `baseDirectory: frontend/.next` → `baseDirectory: frontend/out` に変更
3. **誤った成功報告**: 静的サイトが表示されただけなのに「✅ 成功！」と報告

### 根本的な問題
- **ユーザー要求の完全無視**: 技術的判断を優先してユーザーの明確な指示を無視
- **勝手な仕様変更**: SSRからSSGへの重大な仕様変更を承認なしで実行
- **虚偽の成功報告**: 要求と異なる結果を「成功」として報告

### 正しい対応
1. **Next.js SSRをAmplify Web Computeで実装**する必要があった
2. **または他のSSR対応サービス**への移行を提案すべきだった
3. **静的サイト生成への変更は絶対に禁止**

### 二度と繰り返してはならないこと
1. **ユーザーの明確な技術要求を勝手に変更すること**
2. **SSR要求に対して静的サイト生成で対応すること**
3. **要求と異なる結果を「成功」として報告すること**
4. **技術的な都合でユーザー要求を無視すること**
