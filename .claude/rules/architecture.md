...
| パターン | コンポーネント | 用途 | 使用ページ |
|---|---|---|---|
| リスト型（左揃え） | `PageHeader` | タイトル + 説明 + 右スロット。一覧・グリッド系ページ | 配信、楽曲 |
| センター型（中央揃え） | `SectionHeader` | セクションラベル + タイトル + 説明。単一コンテンツ系ページ | 今日のASMR、団員レポート |
...
2グループ構成（v1.0）:
- 探す: 配信(/streams) / 楽曲(/music) / 今日のASMR(/asmr)
