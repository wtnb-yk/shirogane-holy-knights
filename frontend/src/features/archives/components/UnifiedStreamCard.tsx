...
import { StreamCardProps } from '@/types/components';
 ...
 */
export const UnifiedStreamCard = ({ stream, index, variant = 'default', className }: StreamCardProps) => {
  // バリアント別のスタイル設定
  const variantStyles = {
    default: {
      container: 'group',
    ...
    featured: {
      container: 'group col-span-2 row-span-2',
      card: 'overflow-hidden h-full',
      image: 'aspect-video',
  ...
  return (
    <StaggeredItem index={index} className={cn(currentVariant.container, className)}>
      ...
      >
        {stream.thumbnailUrl ? (
          <div className={cn('relative w-full overflow-hidden bg-black', currentVariant.image)}>
            <Image
          ...
          <div className={cn('w-full bg-bg-accent flex items-center justify-center', currentVariant.image)}>
            <Radio className="w-12 h-12 text-text-tertiary" />
          </div>
        )}
        ...
        <div className="p-4 space-y-3">
          ...
          {(variant === 'featured' || variant === 'pickup') && stream.description && (
...
// 後方互換性のためのエクスポート
export const StreamCard = (props: Omit<StreamCardProps, 'variant'>) => (
  <UnifiedStreamCard {...props} variant="default" />
);

export const FeaturedStreamCard = (props: Omit<StreamCardProps, 'variant'>) => (
  <UnifiedStreamCard {...props} variant="featured" />
);

export const PickupStreamCard = (props: Omit<StreamCardProps, 'variant'>) => (
  <UnifiedStreamCard {...props} variant="pickup" />
);